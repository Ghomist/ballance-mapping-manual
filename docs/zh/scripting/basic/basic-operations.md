...
双击 BB 可以更改它们的参数。修改参数时，若参数输入没有连接到变量，则会自动创建变量。若参数输入已经连接到了其它 BB 的参数输出，或连接到了 Parameter Operation，则不会在参数表中显示该参数。

此外，部分对 BB 右键选择 `Edit Settings`（或选中 BB 后按下 `S` 快捷键）可以调节它们的设置。设置不同于参数，一般是编写脚本时就固定下来的，无法在运行时动态修改。设置有时很重要，例如 Op 的设置就是它对应的运算操作、物体操作类 BB 的设置能够设置。
